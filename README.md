...
```
1. 扫描计划任务、注册表自启动、开始菜单自启动、服务的项目,测试版支持xp以上
2. 扫描dns缓存,测试版支持xp以上
3. 扫描TCP表,测试版支持xp以上
4. 扫描用户列表,测试版支持xp以上
6. 扫描amcache,扫描历史程序启动记录,支持win8以上
7. 扫描登录日志,检测登录主机名、IP、检测RDP爆破,支持win7以上
8. 扫描域控日志,检测hash传递、万能钥匙,支持win7以上
9. PowerShell执行历史记录扫描,支持win7以上
10.[beta测试版]prefetch扫描,获取最近的程序执行记录,支持xp以上
11.*[beta测试版]runmru扫描,获取所有用户最近的通过"win+r运行"执行的程序.
12.*[beta测试版]shimcache扫描,获取最近程序执行记录,支持xp以上
13.*[beta测试版]AppCompatFlags扫描,获取最近程序执行记录,支持win7以上
14.*[beta测试版]Muicache扫描,获取最近程序执行记录,支持xp以上
15. 对以上这些项目对接IOC进行检查,检查hash、IP、域名,标注可疑项目(需要自己申请APIKEY)
16. 支持CSV报表导出
```
